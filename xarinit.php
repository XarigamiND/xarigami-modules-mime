...
function mime_init()
{
    ...
    $mimefile = 'modules/mime/xarincludes/mime.magic.php';
    if (!file_exists($mimefile)) {
        mime_delete();
        $msg = xarML('Could not open #(1) for inclusion', $mimefile);
        throw new FileNotFoundException($mimefile);
    } else {
        include($mimefile);

        if (!isset($mime_list) || empty($mime_list)) {
            mime_delete();
            throw new DataNotFoundException('mime_list');
        }
